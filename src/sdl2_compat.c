...
CreateHintCallbackPropertyName(const char *name, SDL_HintCallback callback, void *userdata) {
    size_t len_property_name = SDL_strlen(name) + 1 + (2 * sizeof(void *) + 2) + 1 + (2 * sizeof(void *) + 2) + 1;
    char *property_name = (char *)SDL3_malloc(len_property_name);
    if (!property_name) {
        return NULL;
    }
    SDL3_snprintf(property_name, len_property_name, "%s_%p_%p", name, (void *)callback, (void *)userdata);
